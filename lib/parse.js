...
define(['esprima', './lang'], function (esprima, lang) {
    ...
    function getFormalParameters(factory) {
        return (factory.params || []).filter(function (param) {
     ...
     *
     * @returns {Object} Wrapped output values:
	 * * dependencies - an array of dependency strings. The dependencies
     *   have not been normalized, they may be relative IDs.
	 * * params - an array with formal parameter names of the dependencies,
	 *   which were used in the factory callback.
	 * * astRoot - parsed JavaScript AST for caching purposes.
     */
    parse.findDependencies = function (fileName, fileContents, options) {
        var modules = [],
            params = [],
			astRoot = typeof fileContents === 'string' ?
					  esprima.parse(fileContents, {range: options && options.range}) : fileContents;
     ...
     * @returns {Object} 
     * @returns {Object} Wrapped output values:
	 * * range - an array with start and end positions of the identifier
	 *   found in the specified text.
	 * * astRoot - parsed JavaScript AST for caching purposes.
     */
    parse.findIdentifier = function (fileName, fileContents, identifier, options) {
		var astRoot = typeof fileContents === 'string' ?
					  esprima.parse(fileContents, {range: true}) : fileContents,
            range;
    ...
    /**
     * Comnverts esprima's range [start,end] to {start,end} positions
     * containing {line,character} numbers.
     * @param {String} fileContents
     * @param {Array} range
     *
     * @returns {Object} Start and end location as objects with line
     * and character properties.
     */
    parse.convertRangeToPositions = function (fileContents, range) {
        ...
        var start = range[0],
            ...
            startLine = getLineNumber(fileContents, 0, start),
            startCharacter = getCharacterNumber(fileContents, 0, start),
            linesInside = getLineNumber(fileContents, start, end),
            endLine = startLine + linesInside,
            endCharacter = linesInside ? getCharacterNumber(fileContents, start, end) : startCharacter + end - start;
